...
public class TypoCoderDiv2 {
    public int count(int[] rating) {
    	...
    	for (int i=1;i<rating.length;i++){
    		if (rating[i]<1200 && flg){
    			res++;
    			flg = false;
    		}else if (rating[i]>=1200 && !flg){
    			res++;
    			flg = true;
    		}
    	}
...
// BEGIN CUT HERE
    public static void main(String[] args) {
        try {
            eq(0,(new TypoCoderDiv2()).count(new int[] {1000,1200,1199}),2);
            eq(1,(new TypoCoderDiv2()).count(new int[] {1500,2200,900,3000}),3);
            eq(2,(new TypoCoderDiv2()).count(new int[] {600,700,800,900,1000,1100,1199}),0);
            eq(3,(new TypoCoderDiv2()).count(new int[] {0,4000,0,4000,4000,0,0}),4);
            eq(4,(new TypoCoderDiv2()).count(new int[] {575,1090,3271,2496,859,2708,3774,2796,1616,2552,3783,2435,1111,526,562}),4);
        } catch( Exception exx) {
